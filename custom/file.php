        ...
        foreach($x->channel->item as $entry) {
            ...
            {  
                ...
                if($rowcount < 1)
                {
                    ...
                    if($q1){
                        echo "Not Found Inserted". "<br><br>";
                    }
                    $a = explode(' ', $str);
                    $model = "SELECT * from oc_pro where pro_id=$a[1]";
                    $model=mysqli_query($conn,$model);
                    $model = mysqli_fetch_assoc($model);
                    $add_pro = "INSERT INTO `oc_product`(`model`,phnum,pro_id ,qnt,`sku`, `upc`, `ean`, `jan`, `isbn`, `mpn`, `location`, `quantity`, `stock_status_id`, `image`, `manufacturer_id`, `shipping`, `price`, `points`, `tax_class_id`, `date_available`, `weight`, `weight_class_id`, `length`, `width`, `height`, `length_class_id`, `subtract`, `minimum`, `sort_order`, `status`, `viewed`, date_added,date_modified	) VALUES ('".$model['pro_name']. "','$mob', '$a[1]',$a[2],'', '', '', '', '', '', '', 1, 7, '" . $model['pro_img'] . "', 0, 1, '$a[3]', 0, 0, '".date("Y-m-d"). "', 0, 1, 0, 0, 0, 1, 1, 1, 1, 1,1,'2009-02-03 16:06:50','2009-02-03 16:06:50')";
